    ...
    <section class="store-nav-wrapper">
        <div class="container store-nav d-flex justify-content-between align-items-center">
            <ul class="nav nav-tabs">
                ...
                <li class="nav-item">
                    <a class="nav-link" href="/loja/{{ $loja->id }}/produtos">Produtos</a>
                </li>
            </ul>
    ...
    <main class="container">
        <section class="store-main-content">
                <div class="btn-voltar" >
                    <img src="{{ asset('icons/icon-voltar.png') }}" onclick="window.history.back()" alt="Voltar" class="icon">
                    Voltar
                </div>
            <div class="row g-4 align-items-center">
                <div class="col-md-7 store-info">
                    <h5><i class="bi bi-geo-alt-fill me-2"></i> Endereço:</h5>
                    <p>{{ $loja->address }}, {{ $loja->number}}<br>{{ $loja->district }}<br>{{ $loja->zip_code }}</p>
...
@push('scripts')
    <script>
        ...
        window.initLojaMap = function() {
            ...
            const loja = {
                name: "{{ $loja->name }}",
                address: "{{ $loja->address }}, {{ $loja->district }} - {{ $loja->city }}",
